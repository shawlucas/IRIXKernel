...
static __uint64_t __string_to_ull(const char* str, char** nptr, int base, int* isneg)
{
	...
	*isneg = neg;
	...
	multmax = ULLONG_MAX / (unsigned)base;
...
/* ANSI 4.10.1.5 */
extern __int64_t strtoll(const char* str, char** nptr, int base)
{
/* assume that abs(LONGLONG_MIN) == LONGLONG_MAX+1 */
#define	ABS_LONGLONG_MIN	(((unsigned long long) LLONG_MAX) + 1)
	...
	if (errno == ERANGE && val == ULLONG_MAX) {
		...
		return((long long)(isneg ? LLONG_MIN : LLONG_MAX));
	} else if (isneg ? (val > ABS_LONGLONG_MIN) : (val > LLONG_MAX)) {
		/* overflow of LONGLONG_MIN/MAX */
		setoserror(ERANGE);
		return(isneg ? (long long)LLONG_MIN : (long long)LLONG_MAX);
	} else {
...
/* ANSI 4.10.1.6 */
extern __uint64_t strtoull(const char* str, char** nptr, int base)
{
    unsigned long long val;
	int isneg;
	int old_errno = errno;
...
/* ANSI 4.10.1.3 */
extern __int64_t atoll(const char* p)
{
